...
# vehicles permanently assigned to depot j and mode t
...
# -------- C6: Global fleet definition --------
cnt = 0
for t in T:
    m.addConstr(
        n[t] == quicksum(N_jt[j, t] for j in J),
        name=f"fleet_definition_{t}"
    )
    cnt += 1
report("(C6) global fleet definition", cnt)


# -------- C6a: Vehicle home-depot ownership --------
cnt = 0
for p in P:
    for j in J:
        for t in T:
            m.addConstr(
                n_jt[j, t, p] <= N_jt[j, t],
                name=f"{p}_vehicle_home_{j}_{t}"
            )
            cnt += 1
report("(C6a) vehicle home-depot ownership", cnt)
