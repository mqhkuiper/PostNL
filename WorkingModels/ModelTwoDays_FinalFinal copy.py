...
if m.SolCount > 0:
    ...
    for p in P:
        ...
        # ============================================================
        # CHECK: implicit inter-depot vehicle reallocation
        # ============================================================

        print("\nIMPLICIT VEHICLE REALLOCATION CHECK (X vs Y):")
        ...
        for t in T:
            for j in J:
                ...
                if abs(diff) > 1e-6:
                    print(f" Mode {t}, depot {j}: {nx:.1f} → {ny:.1f}  (Δ = {diff:+.1f})")

                reallocation_rows.append({
                    "mode": t,
                    "depot": j,
                    "vehicles_day_X": round(nx, 1),
                    "vehicles_day_Y": round(ny, 1),
                    "difference_X_minus_Y": round(diff, 1),
                })
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        pd.DataFrame(reallocation_rows).to_excel(writer, sheet_name="VehicleReallocationCheck", index=False
)
