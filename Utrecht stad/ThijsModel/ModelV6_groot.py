...
this model:
 ...
 - enforces depot-consistent tours (option A)
 - allows footbike to end at last route without returning to depot
'''
...
f_j = {j: 50 for j in J}          # fixed depot opening costs (currently zero)
...
# counter for routes without any depot distances
missing = 0
...
x = m.addVars(A, T, vtype=GRB.INTEGER, lb=0, name="x") #-----> dit moet worden veranderd naar binary er kunnen niet meer vehicles over 1 arc gaan
...
# ============================================================
# 8) constraints (ALL MODES RETURN TO DEPOT)
# ============================================================
...
sol_path = os.path.join(SCRIPT_DIR, "Solution_Large_V6.sol")
m.write(sol_path)
...
if m.SolCount > 0:
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        pd.DataFrame(depot_balance_rows).to_excel(writer, sheet_name="DepotStartEndCheck", index=False)
